...
class SimpleFnEmitter {
    ...
    std::set<Reg> regs_available = {RAX, RBX, RCX, RDX, RDI, RSI, R8, R9, R10, R11, R12, R13, R14, R15};
    ...
    struct [[nodiscard]] RegHolder {
        ...
        RegHolder(Reg reg, ir::Type type, SimpleFnEmitter* emitter) noexcept : reg{reg}, type{type}, emitter{emitter} {}

        RegHolder(const RegHolder&) = delete;
        RegHolder& operator=(const RegHolder&) = delete;

        RegHolder(RegHolder&& other) noexcept : reg{std::exchange(other.reg, std::nullopt)}, type{other.type}, emitter{other.emitter} {}
    ...
    struct [[nodiscard]] asmi {
        struct mov {
            std::variant<std::pair<RegVal, VarLocCode>,
                         std::pair<VarLocCode, RegVal>,
                         std::pair<RegVal, int_t>,
                         std::pair<RegVal, MemLocCode>,
                         std::pair<MemLocCode, RegVal>> val;

            explicit mov(RegVal a, VarLocCode b) : val{std::pair{a, std::move(b)}} {}
            explicit mov(VarLocCode a, RegVal b) : val{std::pair{std::move(a), b}} {}
            explicit mov(RegVal a, int_t b) : val{std::pair{a, b}} {}
            explicit mov(RegVal a, MemLocCode b) : val{std::pair{a, std::move(b)}} {}
            explicit mov(MemLocCode a, RegVal b) : val{std::pair{std::move(a), b}} {}
        };
        struct lea {
            RegVal reg;
            std::variant<ir::StringConstantName, ir::VTableName> val;

            explicit lea(RegVal reg, ir::StringConstantName val) : reg{reg}, val{val} {}
            explicit lea(RegVal reg, ir::VTableName val) : reg{reg}, val{std::move(val)} {}
        };
        struct neg {
            RegVal reg;
        };
        ...
        struct jmp {
            ...
            template<class A>
            explicit jmp(A a) : val{std::move(a)} {}
        };
        struct jz {
            ir::Label target;
        };
        ...
        struct call {
            ...
            explicit call(ir::FnName a) : val{std::move(a)} {}
            explicit call(MemLocCode a) : val{std::move(a)} {}
        };
        using Instr = std::variant<mov, lea, neg, xor_, add, sub, imul, cdq, idiv, test, cmp, jmp, jz, jnz, jl, jle, jg, jge, je, jne, push, add_rsp, call>;
    };
        ...
        std::visit(overloaded{
            [&](const ir::Var& var) {
                ...
                res.preparing_instructions.emplace_back(
                    std::make_unique<AsmiInstrWrapper>(AsmiInstrWrapper{.instr = asmi::mov{reg, var_loc_code(var, ir::Type::PTR)}}));
                back_insert(res.code, to_asm(reg.reg.value(), ir::Type::PTR));
            },
            [&](const ir::Null& /*unused*/) {
                back_insert(res.code, '0');
            },
        }, mloc.base);
        if (mloc.scale != 0) {
            ...
            std::visit(overloaded{
                [&](const ir::Var& var) {
                    ...
                    res.preparing_instructions.emplace_back(
                        std::make_unique<AsmiInstrWrapper>(AsmiInstrWrapper{.instr = asmi::mov{reg, var_loc_code(var, ir::Type::INT)}}));
                    back_insert(res.code, to_asm(reg.reg.value(), ir::Type::PTR));
                },
                [&](int_t i) {
                    back_insert(res.code, i);
                },
            }, mloc.index);
        }
    ...
    void emit_instr(const asmi::Instr& instr) {
        ...
        auto append = [this, &curr_instr](auto& self, const auto& x) {
            ...
            if constexpr (std::is_same_v<XT, ir::Label>) {
                ...
                back_insert(curr_instr, ", [");
                std::visit(overloaded{
                    [&](const ir::StringConstantName& scn) { back_insert(curr_instr, "rel ", to_str(scn)); },
                    [&](const ir::VTableName& vtn) { back_insert(curr_instr, "rel ", to_str(vtn)); },
                }, x.val);
                back_insert(curr_instr, ']');
                ...
                back_insert(curr_instr, "push ", to_asm(x.reg.reg, ir::Type::PTR)); // push as 8 bytes
    ...
    void emit(ir::Instruction& instr) {
        ...
        auto emit_call = [this](const std::variant<ir::FnName, ir::ConstMemLoc>& func, const std::vector<ir::CallArg>& args) {
            ...
            std::visit(overloaded{
                [&](const ir::FnName& fname) {
                    emit_instr(asmi::call{fname});
                },
...
class Emitter {
    ...
    void emit_bultins() {
        ...
        auto emit_label = [this](const char* asm_label) {
            out << asm_label << ":\n";
        };
        ...
        auto arg = [&](unsigned num) {
            return concat("[rbp + ", 16 + 8 * num, ']');
        };
        ...
        emit_builtin_func(ir::builtin_concat_strings, [&] {
            ...
            emit_instr("add rdi, 8");
            emit_instr("call strlen wrt ..plt");
            emit_instr("lea rdi, [rax + 9] ; second str len + 8 for ref count + 1 for trailing null");
        ...
        emit_builtin_func(ir::builtin_destruct_string, [&] {
            emit_instr("mov rdi, ", arg(0));
            emit_instr("; no need to test if rdi != 0 because string reference cannot be null");
